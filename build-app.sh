...
<key>CFBundleShortVersionString</key><string>1.350</string>
<key>CFBundleVersion</key><string>1350</string>
...
ditto -c -k --sequesterRsrc --keepParent "$APP_DIR" "$DIST_DIR/Zebtron-Camera-Zapper-1.350-App.zip"
hdiutil create -volname "Zebtron Camera Zapper 1.350" -srcfolder "$APP_DIR" -ov -format UDZO "$DIST_DIR/Zebtron-Camera-Zapper-1.350.dmg" >/dev/null
echo "$DIST_DIR/$APP_NAME"
echo "$DIST_DIR/Zebtron-Camera-Zapper-1.350-App.zip"
echo "$DIST_DIR/Zebtron-Camera-Zapper-1.350.dmg"
