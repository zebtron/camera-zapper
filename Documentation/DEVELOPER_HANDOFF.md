...
Runtime state lives in `~/Library/Application Support/Zebtron Camera Zapper/` and must never be included in a handoff. Version 1.348 is ad-hoc signed. Public distribution still requires Apple Developer ID signing, hardened runtime/entitlements review, notarization, OAuth production setup, privacy/API verification, and an updater.

## 1.348 pre-release hardening
