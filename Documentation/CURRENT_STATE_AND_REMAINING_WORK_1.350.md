# Zebtron Camera Zapper 1.350 — Current State and Remaining Work

Updated: September 3, 2026

## Current state

Version 1.350 is built as a controlled macOS beta. The app, source, documentation, screenshots, PDF manual, DMG, application ZIP, and checksums are stored together in this handoff.

Implemented in 1.350:

- Flickr's signed live-login check uses the canonical API endpoint, and raw OAuth diagnostics containing identifiers or tokens are suppressed.
...
- Version and build identifiers: 1.350 / 1350.
...
1. Install the 1.350 DMG on the M1 16 GB Mac and test wizard completion, relaunch persistence, settings export, and settings import.
...
6. If testing passes, commit and push the final 1.350 sources to GitHub and create the `v1.350` prerelease with DMG, app ZIP, PDF manual, and checksums.
7. Update `zebtron.com/zapper/` using `Documentation/WEBSITE_UPDATE_HANDOFF_1.350.md` and the two 1.350 screenshots.
...
Start with this file, `Documentation/DEVELOPER_HANDOFF.md`, `Documentation/RELEASE_NOTES_1.350.md`, and the repository status. Do not embed or commit OAuth JSON, Flickr credentials, tokens, passwords, Keychain exports, or personal settings backups. Re-run `swift test` and `./build-app.sh` after code changes, then update `dist/SHA256SUMS.txt`.
