# Camera Zapper Website Update Handoff - Version 1.346

## Purpose

Update `zebtron.com/zapper/` for the 1.346 controlled beta. The site source is in the sibling `CameraZapperWebsite` project. This document is the implementation brief for a person or another LLM updating the hosted site.

## Release wording

Use **Version 1.346 controlled beta**. Do not call the current ad-hoc-signed DMG a production-ready public release. Public distribution remains gated on:
...
1. Change every displayed version and bug-report subject from the previous beta to 1.346.
...
> Install Camera Zapper in Applications, install Android Platform Tools if you use Android, then review the local archive and enable only the destinations you actually use. Start with noncritical media and leave deletion confirmation enabled while testing.
...
- `Zebtron-Camera-Zapper-1.346.dmg`
- `Zebtron-Camera-Zapper-1.346-User-Manual.pdf`
...
- `Documentation/Screenshots/camera-zapper-finished-app.png` - current documentation screenshot
- `Documentation/Zebtron-Camera-Zapper-1.346-User-Manual.pdf` - downloadable manual

## Verification checklist

- Version is consistently 1.346.
