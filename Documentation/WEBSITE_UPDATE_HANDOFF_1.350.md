# Camera Zapper Website Update Handoff - Version 1.350

## Purpose

Update `zebtron.com/zapper/` for the 1.350 controlled beta. The site source is in the sibling `CameraZapperWebsite` project. This document is the implementation brief for a person or another LLM updating the hosted site.

## Release wording

Use **Version 1.350 controlled beta**. Do not call the current ad-hoc-signed DMG a production-ready public release. Public distribution remains gated on:
...
1. Change every displayed version and bug-report subject from the previous beta to 1.350.
...
13. Note that 1.350 is a Flickr OAuth hotfix: the live-login check now uses Flickr's canonical API host and raw provider diagnostics are suppressed to protect OAuth identifiers.
...
- `Zebtron-Camera-Zapper-1.350.dmg`
- `Zebtron-Camera-Zapper-1.350-User-Manual.pdf`
...
- `Documentation/Screenshots/dashboard.png` - tidy generic dashboard
- `Documentation/Zebtron-Camera-Zapper-1.350-User-Manual.pdf` - downloadable manual

## Verification checklist

- Version is consistently 1.350.
