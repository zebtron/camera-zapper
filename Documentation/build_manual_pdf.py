...
OUTPUT = ROOT / "Zebtron-Camera-Zapper-1.348-User-Manual.pdf"
...
def footer(canvas, doc):
    ...
    canvas.drawString(0.7*inch, 0.35*inch, "Zebtron Camera Zapper 1.348 - Controlled Beta")
    canvas.drawRightString(7.8*inch, 0.35*inch, f"Page {doc.page}")
    canvas.restoreState()

doc = SimpleDocTemplate(str(OUTPUT), pagesize=letter, rightMargin=0.7*inch, leftMargin=0.7*inch, topMargin=0.65*inch, bottomMargin=0.68*inch, title="Zebtron Camera Zapper 1.348 User Manual", author="Zebtron")
...
story += [Paragraph("Version 1.348 controlled beta", styles["H2Manual"]), Paragraph("A safety-first workflow for verified Android, camera-card, local, NAS, photo-library, cloud, and catalog destinations.", styles["BodyManual"]), PageBreak()]
