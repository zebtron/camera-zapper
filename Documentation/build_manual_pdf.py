...
OUTPUT = ROOT / "Zebtron-Camera-Zapper-1.347-User-Manual.pdf"
...
def footer(canvas, doc):
    ...
    canvas.drawString(0.7*inch, 0.35*inch, "Zebtron Camera Zapper 1.347 - Controlled Beta")
    canvas.drawRightString(7.8*inch, 0.35*inch, f"Page {doc.page}")
    canvas.restoreState()

doc = SimpleDocTemplate(str(OUTPUT), pagesize=letter, rightMargin=0.7*inch, leftMargin=0.7*inch, topMargin=0.65*inch, bottomMargin=0.68*inch, title="Zebtron Camera Zapper 1.347 User Manual", author="Zebtron")
...
shot = ROOT / "Screenshots" / "dashboard-1.347.png"
if shot.exists():
    image = Image(str(shot)); image._restrictSize(7.05*inch, 3.72*inch); story += [image, Spacer(1, 0.22*inch)]
story += [Paragraph("Version 1.347 controlled beta", styles["H2Manual"]), Paragraph("A safety-first workflow for verified Android, camera-card, local, NAS, photo-library, cloud, and catalog destinations.", styles["BodyManual"]), PageBreak()]
wizard_shot = ROOT / "Screenshots" / "setup-wizard-1.347.png"
...
for line in lines:
    ...
    if s.startswith("### "):
        flush(); story.append(Paragraph(inline(s[4:]), styles["H2Manual"])); continue
    if re.match(r"^\d+\. ", s):
